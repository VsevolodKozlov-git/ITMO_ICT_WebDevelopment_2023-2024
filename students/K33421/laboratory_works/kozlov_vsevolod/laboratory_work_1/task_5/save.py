...
class MyHTTPServer:
    ...
    def handle_request(self, request:Request) -> Response:
        ...
        if request.method == 'POST' and request.url_path =='/marks':
            return self.handle_post_mark(request)
        if request.method == 'GET' and request.url_path == '/marks':
            return self.handle_get_mark(request)
        if request.method == 'GET' and request.url_path == "/update_marks":
            return self.handle_update_marks(request)
        if request.method == 'GET' and request.url_path == '/favicon.ico':
            return Response(200, 'OK')

        return self.handle_404(request)

    def handle_post_mark(self, request: Request):
    ...
    def handle_update_marks(self, request):
